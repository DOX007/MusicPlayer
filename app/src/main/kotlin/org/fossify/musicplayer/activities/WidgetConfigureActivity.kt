...
class WidgetConfigureActivity : SimpleActivity() {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val isCustomizingColors =
            intent.extras?.getBoolean(IS_CUSTOMIZING_COLORS) == true
        ...
        binding.configPlayer.apply {
            ...
            songInfoTitle.text =
                currSong?.title ?: getString(R.string.no_track_playing)
        }
    }
    ...
    private fun initVariables() {
        ...
        if (
            mTextColor ==
            resources.getColor(org.fossify.commons.R.color.default_widget_text_color)
            && isDynamicTheme()
        ) {
            mTextColor = resources.getColor(
                org.fossify.commons.R.color.you_primary_color,
                theme
            )
        }
    ...
    private fun pickBackgroundColor() {
        ColorPickerDialog(this, mBgColorWithoutTransparency) { ok, color ->
            if (ok) {
    ...
    private fun pickTextColor() {
        ColorPickerDialog(this, mTextColor) { ok, color ->
            if (ok) {
                mTextColor = color
                updateTextColor()
            }
